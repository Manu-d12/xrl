...
export default class extRelList extends NavigationMixin(LightningElement) {
	...
	setConfig(cmd, data) {
		...
		try{
			const labels = this.config._LABELS !== undefined ? this.config._LABELS.title_expandView.split('/') : 'Expand View / Collapse View'.split('/');
			const icons = this.config._LABELS !== undefined ? this.config._expandIcon.split('/') : "utility:expand/utility:contract".split('/');

			expandAction.actionTip = this.isFullscreen ? labels[1] : labels[0];
			expandAction.actionIconName = this.isFullscreen ? icons[1] : icons[0];
		}catch(e){
			console.error(e);
		}
		this.config.actionsBar = {
		...
		this.config.isShowConfigure = true; //putting the default value
		if(this.config?._advanced?.isShowConfigureCallback){
			try{
				this.config.isShowConfigure = this.config._advanced.isShowConfigureCallback(this,libs);
			}catch(e){
				this.config._errors = libs.formatCallbackErrorMessages(e,'table','Show Configure Callback');
			}
		}
	...
	async prepareFieldsToFetch(){
		...
		this.config.listViewConfig[0]?.colModel?.forEach(e => {
			...
			if (e.type === 'reference') {
				...
				let prefix = e.fieldName.split('.');
				prefix.splice(-1, 1);
				this.config.fields.push(e.relationshipName ? (prefix.length> 0 ? prefix.join(".") +'.' : '') + e.relationshipName + nameField : e.fieldName);
				if (e.locked) this.config.lockedFields.push(e.relationshipName ? e.relationshipName + '.Name' : e.fieldName);
			}
	...
	handleFlowStatusChange(event) {
		...
		if(event.detail.status.toUpperCase() === 'ERROR') {
