	...
	get colModelItem() {
		...
		for (let item in tmp) {
			...
			result.push({
				"paramName" : item,
				"type" : tmp[item].type,
				"label" : tmp[item].helpArticleUrl === undefined || tmp[item].isAdvanced ? tmp[item].label : '',
				"isTextArea" : (tmp[item].type === 'function'),
				"tooltip" : tmp[item].helpArticleUrl === undefined || tmp[item].isAdvanced ? (item === 'fieldName') ? tmp[item].tooltip + '\n' + '.Field Type:' + fieldParams.type : tmp[item].tooltip : '',
	...
	get tableItem() {
		...
		for (let item in tmp) {
			...
			result.push({
				...
				"label" : tmp[item].helpArticleUrl === undefined || tmp[item].isAdvanced ? tmp[item].label : '',
				"isTextArea" : (tmp[item].type === 'function'),
				"tooltip" : tmp[item].helpArticleUrl === undefined || tmp[item].isAdvanced ? tmp[item].tooltip : '',
				//"isDisabled" : (item === 'fieldName'),
				"value" : defValue,
				"isChecked" : (tmp[item].type === 'checkbox') ? defValue : undefined,
